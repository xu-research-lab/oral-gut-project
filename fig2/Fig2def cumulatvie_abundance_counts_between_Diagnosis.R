...
library(dplyr)
library(ggpubr)
load("../data/Figure2/shared_abundance_counts.RData")
gut_boxplot_shared<-ggplot(subset(shared_abundance_counts,treatment=="non-PMA"&Diagnosis!="Treated"),aes(Diagnosis,gut_sum))+
...
gut_boxplot_shared
ggsave(filename = "../results/fig2e.pdf",plot = gut_boxplot_shared,width = 3,height = 2.2)

gut<- ggplot(subset(shared_abundance_counts,treatment=="non-PMA"&Diagnosis!="Treated"),aes(oral_sum,gut_sum,color=Diagnosis))+
  geom_point()+theme_bw()+
...
gut
ggsave(filename = "../results/fig2f.pdf",plot = gut,width = 3.75,height = 2.6)
oral_gut_shared_SGB_counts<-ggplot(subset(shared_abundance_counts,treatment=="non-PMA"&Diagnosis!="Treated"),aes(Diagnosis,shared_counts))+
  geom_boxplot(aes(color=Diagnosis),show.legend = F,outlier.shape = NA)+
  #geom_point(aes(color=Diagnosis),width = 0.1,show.legend = F)+
  geom_jitter(aes(color=Diagnosis),width = 0.05,height = 0,show.legend = F)+
  theme_bw()+
...
ggsave(filename = "../results/fig2d.pdf",plot = oral_gut_shared_SGB_counts,width = 2.8,height = 2.2)
